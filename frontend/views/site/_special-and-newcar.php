...
?>
    ...
    <div class="span9">
        <ul class="breadcrumb">
            ...
            <li class="active"><?= Yii::$app->controller->action->id == 'special-offer' ? 'Specials Offer' : 'Newcars' ?></li>
        </ul>
            ...
            <ul>
                <li class="<?= count($pages) == 1 || $p == 1 ? 'active' : '' ?>">
                    <a onclick="<?= count($pages) == 1 || $p == 1 ? 'return false;' : '' ?>"
                       href="<?= $p > 1 ? $baseUrl . '/'.$ctrl.'?p=' . ($p-1) : '#' ?>" >‹</a>
                </li>
                <?php foreach($pages as $key => $status) : ?>
                    <li class="<?= $status ?>"><a onclick="<?= $status == 'active' ? 'return false;' : '' ?>" href="<?= $status == '' ? $baseUrl . '/'.$ctrl.'?p=' . ($key+1) : '#' ?>"><?= $key+1 ?></a></li>
                <?php endforeach; ?>
                <li class="<?= count($pages) == 1 || $p == $totalPage ? 'active' : '' ?>">
                    <a onclick="<?= count($pages) == 1 || $p == $totalPage ? 'return false;' : '' ?>"
                       href="<?= $p < $totalPage ? $baseUrl . '/'.$ctrl.'?p=' . ($p+1) : '#' ?>" >›</a>
                </li>
            </ul>
        </div>
    </div>
